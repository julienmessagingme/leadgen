...
var DEFAULT_EMAIL_TEMPLATE =
  ...
  "3. APPORTER DE LA VALEUR + CITER LES CAS CLIENTS FOURNIS : un insight concret, une tendance, ou un retour d experience sur comment le conversationnel resout un probleme specifique au secteur/taille du prospect. Si des cas clients sont fournis dans le contexte (champ 'Cas clients supplementaires a citer si pertinents'), cite-les TOUS (pas juste le 1er) — nom du client + chiffre + 1 phrase de lien avec le conversationnel. Ne cite JAMAIS un cas absent du contexte. Tu peux dire 'le messaging conversationnel' ou 'WhatsApp Business' ou 'chatbot IA'. Pas un pitch produit.\n" +
...
var DEFAULT_EMAIL_FOLLOWUP_TEMPLATE =
  ...
  "2. ANGLE DIFFERENT du 1er email — SANS FAKE REFLEXION : ne re-cite PAS le signal initial (like/commentaire sur un post). Pars sur un cas client concret et croise avec le metier/taille du prospect via l angle conversationnel. ZERO phrase du type 'votre post m a fait reflechir', 'ca m a fait penser a', 'en lisant X j ai pense Y', 'ca m a rappele', 'je suis tombe sur'. On ouvre DIRECTEMENT sur une observation de fond.\n" +
  "   BON EXEMPLE : 'WhatsApp rejoint souvent la pile CRM dans l assurance — un cas concret : [client + chiffre].'\n" +
  "   MAUVAIS EXEMPLE : 'En repensant a votre sujet, ca m a rappele que [client].'\n" +
  "3. CITER TOUS LES CAS CLIENTS FOURNIS : des cas peuvent etre fournis via 'Cas client a citer' ET 'Cas clients supplementaires a citer si pertinents'. Si PLUSIEURS cas sont fournis, cite-les TOUS (pas juste le 1er) — pour chaque : nom du client + chiffre + 1 phrase de contexte + lien explicite avec le conversationnel (quel canal, quel use-case). Si un seul cas est fourni, meme format mais sur un seul. Si AUCUN cas n est fourni, parle d une tendance generale SANS inventer de chiffres precis. Ne cite JAMAIS un cas absent du contexte.\n" +
  ...
  "8. EN FRANCAIS si le prospect est en France, EN ANGLAIS si zone GCC/international.\n" +
  "9. INTERDICTIONS ABSOLUES — SURVEILLANCE & FAKE REFLEXION :\n" +
  "   Stalking : 'j ai vu que vous avez like/commente/reagi', 'vous avez reagi a mes posts', 'vos interactions recentes', 'votre activite recente'.\n" +
  "   Fake reflexion (aussi critique) : 'm a fait reflechir', 'ca m a fait penser', 'm a rappele', 'en lisant', 'votre post m interpelle', 'je suis tombe sur', 'votre post de X sur Y m a/nous a'. On n evoque JAMAIS notre reflexion declenchee par leur contenu — c est du fake cold IA.\n" +
